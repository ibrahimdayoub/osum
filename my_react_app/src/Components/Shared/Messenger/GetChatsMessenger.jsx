...
const GetChatsMessenger = ({ isOpenContacts, currentChat, setCurrentChat, latestMessage }) => {
                    ...
                    chatsState.chats.map((chat) => {
                        return !chat.is_group && chat.users_ids.length > 1 && (
                            <div onClick={() => { setCurrentChat(chat) }} className={
                                ...
                                <div
                                    style={{ backgroundImage: `url(${(chat.users_ids.filter((user) => { return user._id !== authState.auth.user._id }))[0].picture_personal_profile ? process.env.REACT_APP_BACK_END_URL + (((chat.users_ids.filter((user) => { return user._id !== authState.auth.user._id }))[0].picture_personal_profile).substring(7)).replace("\\", "/") : null})` }}
                                    ...
                                    {
                                        !(chat.users_ids.filter((user) => { return user._id !== authState.auth.user._id }))[0].picture_personal_profile ?
                                ...
                                <div className={
                                    ...
                                    <div className="w-full flex items-center text-left">
                                        <span className="text-dark dark:text-light flex-1 text-sm truncate w-2">
                                            {
                                                (chat?.users_ids?.filter((user) => { return user._id !== authState.auth.user._id }))[0].role === "Admin" ?
                                                    `${(chat?.users_ids?.filter((user) => { return user._id !== authState.auth.user._id }))[0].first_name} ${(chat?.users_ids?.filter((user) => { return user._id !== authState.auth.user._id }))[0].last_name} (Admin)` :
                                                    `${(chat?.users_ids?.filter((user) => { return user._id !== authState.auth.user._id }))[0].first_name} ${(chat?.users_ids?.filter((user) => { return user._id !== authState.auth.user._id }))[0].last_name}`
                                            }
                                        </span>
                                        {
...
export default GetChatsMessenger