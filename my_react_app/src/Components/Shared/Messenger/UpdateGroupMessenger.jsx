...
import { updateGroup } from '../../../Redux/Features/Chats/chatsSlice';
import { FaCrown, FaEdit, FaList, FaSpinner, FaTimes } from 'react-icons/fa';
import { toastify } from '../../../Helper';

const UpdateGroupMessenger = ({ isOpenContacts, setSearchParams, currentChat, setCurrentChat, updatedChat, setUpdatedChat }) => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const usersState = useSelector((state) => state.users);
    const chatsState = useSelector((state) => state.chats);
    const [searchUserId, setSearchUserId] = useState("");
    const [groupName, setGroupName] = useState(currentChat.chat_name);
    const [groupUsers, setGroupUsers] = useState(currentChat.users_ids);
    ...
    const updateGroupSubmit = (e) => {
        ...
        dispatch(updateGroup({ id: updatedChat, data: formData }))
    }
    useEffect(() => {
        if ((Date.now() - chatsState.time < 100) && chatsState.status === 200 && chatsState.message && chatsState.operation === "updateGroup") {
        ...
        else if ((Date.now() - chatsState.time < 100) && (chatsState.status === 400 || chatsState.status === 401 || chatsState.status === 403) && chatsState.message && chatsState.operation === "updateGroup") {
            toastify("error", chatsState.status, chatsState.message)
        }
        else if ((Date.now() - chatsState.time < 100) && chatsState.status === 500 && chatsState.message && chatsState.operation === "updateGroup") {
    ...
    return (
        <div className="text-dark/50 dark:text-light/50 text-left text-xs sm:text-lg py-1 px-1 ml-1">
            <form encType="multipart/form-data" onSubmit={(e) => { updateGroupSubmit(e) }}>
                <div>
                    <div className="mb-1">
                ...
                >
                    {
                        chatsState.isLoading && (chatsState.operation === "createGroup" || chatsState.operation === "updateGroup") ? <FaSpinner className="inline text-xl animate-spin" /> :
                            <>
                                ...
                                {
                                    isOpenContacts ?
                                        <span className="inline text-xs md:text-sm">Update Group {groupUsers.length > 0 ? `(${groupUsers.length})` : null}</span> :
                                        <span className="hidden sm:inline text-xs md:text-sm">Update Group {groupUsers.length > 0 ? `(${groupUsers.length})` : null}</span>
                                }
                            </>
                    }
                </button>
...
export default UpdateGroupMessenger